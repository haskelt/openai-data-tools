...
class DataCoder (DataProcessor):
    ...
    def _configure_examples(self, examples):
        ...
        if examples:
            for example in examples:
                ...
                self._example_messages.append({'role': 'assistant', 'content': json.dumps({
    ...
    def _init_output(self, n_items):
        ...
        self._explanations = [None] * n_items
    
    # Takes <response> and stores it as part of the output data for item <i>
    def _store_output(self, i, response):
        output = json.loads(response['choices'][0]['message']['content'])
        self._output[i] = (np.array(output['answer'].lower().rstrip('.')) == 'yes').astype(dtype=int).tolist()
        self._explanations[i] = output['explanation']
